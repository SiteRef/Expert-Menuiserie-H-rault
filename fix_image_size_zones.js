...
function generateTwoColumnZonesSectionWithLargeImage(citySlug) {
                ...
                <div class="grid grid-2" style="gap: var(--spacing-2xl); align-items: start;">
                    ...
                    <div class="zones-left-column">
                        <div class="zones-content">
                            <div class="zones-info">
                                <img src="../../Images/Nos%20Zones%20d'Intervention%20dans%20l'Hérault.jpg"
                                     ...
                                     style="width: 100%; height: auto; max-width: 100%; border-radius: var(--border-radius); box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); margin-bottom: var(--spacing-lg);">

                                <div class="zones-stats" style="display: flex; justify-content: space-around; margin-bottom: var(--spacing-xl);">
                                    <div class="stat-item" style="text-align: center;">
                                        <span class="stat-number" style="display: block; font-size: 2rem; font-weight: bold; color: var(--primary-blue);">12+</span>
                                        <span class="stat-label" style="font-size: 0.9rem; color: var(--text-muted);">Quartiers couverts</span>
                                    </div>
                                    <div class="stat-item" style="text-align: center;">
                                        <span class="stat-number" style="display: block; font-size: 2rem; font-weight: bold; color: var(--primary-blue);">24h</span>
                                        <span class="stat-label" style="font-size: 0.9rem; color: var(--text-muted);">Délai intervention</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-top: var(--spacing-xl);">
                            <h3 style="color: var(--primary-blue); margin-bottom: var(--spacing-lg);">Communes Limitrophes</h3>
                            <div class="cities-grid" style="grid-template-columns: repeat(2, 1fr); gap: var(--spacing-md);">`;
        ...
        html += `
                                <div class="city-item">
                                    <a href="../${communeSlug}/" style="text-decoration: none; color: inherit;">
                                        <div style="padding: var(--spacing-md); border: 1px solid var(--border-light); border-radius: var(--border-radius); text-align: center; transition: all 0.3s ease; background: white;">
                                            <strong style="color: var(--text-dark);">${displayName}</strong><br>
                                            <span style="color: var(--text-muted); font-size: 0.9rem;">${postalCode}</span>
                                        </div>
                                    </a>
                                </div>`;
    });
    ...
    html += `
                    ...
                    <div class="zones-right-column">
                        <h3 style="color: var(--primary-blue); margin-bottom: var(--spacing-lg);">Quartiers &amp; Zones Couverts</h3>`;

    // Add quartiers sections
    Object.entries(quartiers).forEach(([category, items]) => {
        html += `
                        <div style="margin-top: var(--spacing-xl);">
                            <h4 style="color: var(--primary-blue); margin-bottom: var(--spacing-lg); font-size: 1.1rem;">${category}</h4>
                            <div class="cities-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-md);">`;

        items.forEach(([name, description]) => {
            html += `
                                <div class="city-item">
                                    <div style="padding: var(--spacing-md); border: 1px solid var(--border-light); border-radius: var(--border-radius); background: white; transition: all 0.3s ease;">
                                        <strong style="color: var(--text-dark); display: block; margin-bottom: 4px;">${name}</strong>
                                        <span style="color: var(--text-muted); font-size: 0.9rem;">${description}</span>
                                    </div>
                                </div>`;
        });
...
function processFile(filePath, citySlug) {
    try {
        ...
        // Generate two-column zones section with large image
        const zonesHTML = generateTwoColumnZonesSectionWithLargeImage(citySlug);
...
console.log('Updating to larger image in left column...');
    ...
    if (dir.isDirectory()) {
        ...
        if (fs.existsSync(indexPath)) {
            ...
            if (processFile(indexPath, dir.name)) {
                processedCount++;
                console.log(`✓ ${dir.name} - Large image layout updated`);
            } else {
