...
function generateCompleteZonesSection(citySlug) {
            ...
            <div class="container">
                ...
                <div class="zones-content">
                    <div class="zones-info">
                        <img src="../../Images/Nos%20Zones%20d'Intervention%20dans%20l'Hérault.jpg" alt="Carte intervention ${cityDisplayName}" class="intervention-map">
                ...
                <div style="margin-top: var(--spacing-xl);">
                    ...
                    <div class="cities-grid" style="grid-template-columns: repeat(4, 1fr); gap: var(--spacing-md); margin-top: var(--spacing-lg);">`;
        ...
        html += `
                        <div class="city-item">
                            <a href="../${communeSlug}/" style="text-decoration: none; color: inherit;">
                                <div style="padding: var(--spacing-md); border: 1px solid var(--border-light); border-radius: var(--border-radius); text-align: center;">
                                    <strong>${displayName}</strong><br>
                                    <span style="color: var(--text-muted);">${postalCode}</span>
                                </div>
                            </a>
                        </div>`;
    });
    ...
    Object.entries(quartiers).forEach(([category, items]) => {
        html += `
                    <div style="margin-top: var(--spacing-xl);">
                        <h4 style="color: var(--primary-blue); margin-bottom: var(--spacing-lg);">${category}</h4>
                        <div class="cities-grid" style="grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: var(--spacing-md);">`;

        items.forEach(([name, description]) => {
            html += `
                            <div class="city-item">
                                <div style="padding: var(--spacing-md); border: 1px solid var(--border-light); border-radius: var(--border-radius);">
...
function processFile(filePath, citySlug) {
    try {
        ...
        content = content.replace(/<h3>Quartiers &amp; Zones Couverts<\/h3>[\s\S]*?(?=<\/section>|<!-- |$)/g, '');
        ...
        // Generate complete zones section
        const zonesHTML = generateCompleteZonesSection(citySlug);
...
console.log('Restoring complete Zones d\'Intervention sections...');
    ...
    if (dir.isDirectory()) {
        ...
        if (fs.existsSync(indexPath)) {
            ...
            if (processFile(indexPath, dir.name)) {
                processedCount++;
                console.log(`✓ ${dir.name} - Successfully restored`);
            } else {
                console.log(`✗ ${dir.name} - Failed to restore`);
            }
        }
    }
});
